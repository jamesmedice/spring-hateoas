...
public class AuthorControllerTest extends AbstractControllerTest {
	...
	when().get("/authors/martin_fowler").then().statusCode(HttpStatus.SC_OK).and().contentType(ContentType.JSON).and()
		.body("firstName", equalTo(expectedAuthor.getFirstName())).and().body("lastName", equalTo(expectedAuthor.getLastName()));
	...
	final ExtractableResponse<Response> extractable = given().contentType(ContentType.JSON).body(newAuthorRequestJson).when().post("/authors").then().assertThat()
		.statusCode(201).and().header("Location", containsString("/authors/")).and().extract();
