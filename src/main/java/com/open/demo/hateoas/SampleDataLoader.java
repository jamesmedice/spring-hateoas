...
public class SampleDataLoader implements ApplicationListener<ContextRefreshedEvent> {
	...
	if (bookRepository.count() == 0 && publisherRepository.count() == 0 && authorRepository.count() == 0) {
	    final Publisher[] publishers = { publisherRepository.save(new Publisher("O'Reilly")), publisherRepository.save(new Publisher("Addison Wesley")) };

	    final Author[] authors = { authorRepository.save(new Author("David", "Flanagan")), authorRepository.save(new Author("Martin", "Fowler")),
		    authorRepository.save(new Author("Kendall", "Scott")) };

	    final Book[] books = { bookRepository.save(new Book("0596007736", "Java in a Nutshell", asList(authors[0]), publishers[0], 3)),
